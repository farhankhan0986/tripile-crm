...
async function getCustomer(id, userId, role) {
  ...
  if (role === 'agent' && customer.assignedAgent?._id?.toString() !== userId) return null;
        ...
        <div className="col-span-2 bg-white border border-gray-200 rounded-xl p-5">
          ...
          <dl className="grid grid-cols-2 gap-x-6 gap-y-4 text-sm">
            ...
            <div>
              ...
              <dd className="font-medium text-gray-900">{customer.assignedAgent?.name || '—'}</dd>
            </div>
            <div>
              <dt className="text-gray-500 mb-1">Created By</dt>
              <dd className="font-medium text-gray-900">{customer.createdBy?.name || '—'}</dd>
            </div>
            <div>
              <dt className="text-gray-500 mb-1">Created At</dt>
              <dd className="font-medium text-gray-900">{new Date(customer.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</dd>
            </div>
          </dl>
          {customer.notes && (
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-semibold text-gray-900">{bookings.filter(b => b.status === 'pending').length}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-gray-500">Confirmed</span>
              <span className="font-semibold text-gray-900">{bookings.filter(b => b.status === 'confirmed').length}</span>
            </div>
          </div>
        </div>
      </div>
